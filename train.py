...
if __name__ == '__main__':
    ...
    logger.info('Observation shape : ' + f'{(env.observation_spec().shape[0])}')
    logger.info('Action shape : ' + f'{(env.action_spec().shape[0])}')
    logger.info('Parameters : ' + f'{config_args}')
    ...
    # maximum number of steps in a trajectory to prevent infinite loops
    max_steps = config_args['max_steps_per_episode']
    ...
    for i in range(1, n_trajectories+1):
        ...
        while timestep.step_type != StepType.LAST and n_steps <= max_steps:
            ...
            logger.debug('action ' + f'{action[0]}')
